...
local function updateBaseplateHud(data)
    ...
    if IsPauseMenuActive() then
        -- Hide the HUD if the pause menu is active
        SendNUIMessage({
            action = 'baseplate',
            show = false
        })
...
CreateThread(function()
    ...
    while true do
        ...
        if IsPauseMenuActive() then
            ...
            Wait(200)  -- Avoid unnecessary processing while the menu is open
        else
            local camRot = GetGameplayCamRot(0)
            -- Determine whether the compass follows the camera or the player entity
            ...
            -- Normalize heading to avoid 360, which should be 0
            if heading == 360 then heading = 0 end
            
            -- Convert heading to cardinal direction using getHeadingDirection
            local headingDirection = getHeadingDirection(heading)
            
            -- Only update if heading has changed
            if heading ~= lastHeading then
                if cache.vehicle then
                    local crossroads = getCrossroads(cache.ped)
                    
                    -- Send updated heading direction and show compass
                    SendNUIMessage({
                        action = 'update',
                        value = headingDirection  -- Send direction instead of degrees
                    })
...
-- Function to convert heading to direction (8 cardinal directions)
...
-- Modify the heading logic to use directions (N, NE, E, etc.)
local heading = math.floor(GetEntityHeading(vehicle))  -- Get heading in degrees
if vehicle == 0 then
    heading = math.floor(GetEntityHeading(playerPed))  -- Get heading in degrees for player
end

-- Convert the heading to a direction using getHeadingDirection function
local headingDirection = getHeadingDirection(heading)

-- Now headingDirection will hold one of the 8 directions (N, NE, E, SE, S, SW, W, NW)
